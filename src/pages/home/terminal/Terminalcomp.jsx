...
const Terminalcomp = () => {
  ...
  const processCommand = async e => {
    ...
    switch (command) {
      ...
      case "shiva":
        output =
          "Hello there! I am Shiva, a Full Stack Developer experienced in the MERN stack, currently pursuing my Bachelor's  in Computer Science and Engineering. Type about to know more about me"
        break
      case "neofetch":
        window.location.href =
          "https://www.youtube.com/watch?v=Rl1ImG2b1k8&t=51s"
        output = ""
        break
      case "about":
        router.push("/about")
  ...
  return (
    <div className="border-2  border-black/30 dark:border-white/30 rounded-lg h-[450px] overflow-y-auto w-full">
      ...
      <div className="p-2">
        <h1 className="text-sm font-medium  opacity-70 tracking-wide">
          Get started by typing `help` command below
        </h1>
        <span className=" text-xs capitalize font-medium opacity-70 ">
          Linux enthusiasts i have added a few easter eggs take time to discover
        </span>
        <br />
        <span className=" italic mb-3 text-xs font-medium opacity-60">
          Tip : Type any of my project name to navigate to their respective
          hosted links
        </span>
        {commands.map(command => (
          <div
            ref={terminalRef}
            key={command.id}
            className="mt-2 font-mono flex  justify-between"
          ...
          >
            <div className=" w-full">
              ...
              <span className="text-sm w-full font-medium opacity-70">
                {command.output}
              </span>
            </div>
